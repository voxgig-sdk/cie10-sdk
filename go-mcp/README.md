...
[MCP](https://modelcontextprotocol.io) server exposing the Cie10 SDK as
two agent tools — `cie10_list` and `cie10_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cie10-mcp)
make build

# 2. Provide credentials via the environment
export CIE10_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cie10 \
  -- /absolute/path/to/cie10-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cie10-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cie10_list: first page of records
{ "entity": "cie_10" }
{ "entity": "cie_10", "query": { } }

// cie10_load: one record by id
{ "entity": "cie_10", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cie10-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cie10 \
     -- "$PWD"/dist/*/cie10-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cie10_list` and `cie10_load` tools now appear
   in new sessions. Ask the agent to *"list cie_10 using cie10"*
   and it calls `cie10_list` with `{"entity":"cie_10"}`.
...
```sh
export CIE10_APIKEY=sk_live_xxx            # API key
export CIE10_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cie10_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cie10_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CIE10_APIKEY` | API key sent with every request. |
| `CIE10_BASE` | Optional override of the API base URL. |
...
```sh
./cie10-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cie10_load","arguments":{"entity":"cie_10","query":{"id":1}}}}'
```
