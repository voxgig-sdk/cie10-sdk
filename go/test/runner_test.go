...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CIE10_TEST_LIVE") == "TRUE" ||
		os.Getenv("CIE10_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CIE10_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.Cie10SDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.Cie10Error{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.Cie10SDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.Cie10Error{Msg: msg, Code: code}
}
