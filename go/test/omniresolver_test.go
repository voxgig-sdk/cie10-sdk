...
import (
	...
	omni "github.com/voxgig-sdk/cie10-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Cie10Error); is {
				out := map[string]any{
					"name":    "Cie10Error",
...
func omniCtx(arg any, client *sdk.Cie10SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Cie10SDK); is {
			client = live
