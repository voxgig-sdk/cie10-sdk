...
//
// NewCie10SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.Cie10SDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
...
func fcCandidates(client *sdk.Cie10SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.Cie10SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.Cie10SDK, name string) any {
