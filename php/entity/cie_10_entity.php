...
class Cie10Entity
{
    ...
    {
        $opts = $this->_entopts;
        return new Cie10Entity($this->_client, $opts);
    }

    /**
     * @param Cie10|array $args Cie10 data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = Cie10Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Cie10|array The current Cie10 data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Cie10 fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = Cie10Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Cie10 fields).
     */
    ...
    /**
     * List Cie10 items matching the given filter.
     *
     * @param Cie10ListMatch|array|null $reqmatch Match filter (any subset
     *   of Cie10 fields) as an assoc-array; Cie10ListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Cie10[]|array A list of Cie10 items as assoc-arrays at
     *   the SDK boundary; throws Cie10Error on failure (item-5 convention).
     */
