...
//    only - a typed Cie10Context there would make every ctx
...
final class Cie10Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new Cie10Context($clean, null);
    ...
    public static function err_from_map($m): ?Cie10Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new Cie10Error(is_string($code) ? $code : '', $msg);
    }
