...
// Cie10 SDK primary utility test
//
...
//   Cie10Omni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with Cie10Omni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   Cie10Omni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = Cie10Omni::makeRunner(
                self::TEST_JSON_FILE, Cie10SDK::test(null, null));
    ...
    private static function client(): Cie10SDK
    {
    ...
    {
        $client = Cie10SDK::test(null, null);
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = Cie10Omni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = Cie10Omni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = Cie10Omni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    {
        $hook_client = Cie10SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends Cie10BaseFeature {
            public $hook_fn = null;
            public function TestHook(Cie10Context $ctx): void
            {
    ...
    {
        $init_client = Cie10SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends Cie10BaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new Cie10Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return Cie10Omni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = Cie10SDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = Cie10Omni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof Cie10Error)
