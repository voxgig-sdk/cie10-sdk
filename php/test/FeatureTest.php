...
class FtCtrl extends Cie10Control
{
...
class FtClient
{
    ...
    private Cie10Utility $utility;

    public function __construct(Cie10Utility $utility, array $options)
    {
    ...
    public function get_utility(): Cie10Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Cie10Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (Cie10Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Cie10Config::make_feature($name);
    ...
    private static function buildUrl(Cie10Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Cie10Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Cie10Response($fetched);
        }
    ...
    private function fail(Cie10Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Cie10Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
