...
class Cie10SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = Cie10Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = Cie10Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!Cie10Features::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, Cie10Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return Cie10Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = Cie10Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = Cie10Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = Cie10Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new Cie10Error($op . "_allow",
                "Cie10SDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = Cie10Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new Cie10Error("graphql_error",
                "Cie10SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_cie_10 = null;

    // Canonical facade: $client->Cie10()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->cie_10()
    // resolves here too.
    public function Cie10($data = null)
    {
        require_once __DIR__ . '/entity/cie_10_entity.php';
        if ($data === null) {
            if ($this->_cie_10 === null) {
                $this->_cie_10 = new Cie10Entity($this, null);
            }
            return $this->_cie_10;
        }
        return new Cie10Entity($this, $data);
    }
