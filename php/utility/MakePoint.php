...
// Cie10 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Cie10MakePoint
{
    public static function call(Cie10Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Cie10Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Cie10Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Cie10Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
