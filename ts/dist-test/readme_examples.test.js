...
const TEST_SEED = { "entity": { "cie_10": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+Cie10SDK\b/.test(code) ||
        /\bCie10SDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+Cie10SDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/Cie10SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
