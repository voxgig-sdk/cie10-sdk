// Command Cie10-cli is a CLI + REPL for the Cie10 SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CIE10_APIKEY (api key), CIE10_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CIE10_APIKEY carries the API key and
	// CIE10_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CIE10_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CIE10_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCie10SDK(opts)
