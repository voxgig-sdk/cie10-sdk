...
boru-driven command-line client **and** interactive REPL for the Cie10
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cie10-cli)
make build
...
./cie10-cli list cie_10

# 5. Override the API base URL for a single call
CIE10_BASE=https://api.example.com ./cie10-cli list cie_10
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cie10-cli
   ```
...
Run with no arguments to open a REPL (prompt `cie10>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cie_10`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cie10-cli`. |
...
(`./cie10-cli <expr>`) and interactively (the REPL), and expressions compose the
