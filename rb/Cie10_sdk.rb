...
class Cie10SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = Cie10Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = Cie10Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, Cie10Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = Cie10Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = Cie10Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = Cie10Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = Cie10Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = Cie10Error.new(
        "graphql_error", "Cie10SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Cie10.list / client.Cie10.load({ "id" => ... })
  def Cie10(data = nil)
    require_relative 'entity/cie_10_entity'
    Cie10Entity.new(self, data)
  end
