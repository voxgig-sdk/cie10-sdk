...
class Cie10Entity
  ...
  def make
    opts = @_entopts.dup
    Cie10Entity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = Cie10Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Cie10, Hash] the current Cie10 data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = Cie10Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Cie10 fields)
  def match_get
  ...
  #
  # @param reqmatch [Cie10ListMatch, Hash, nil] match filter (any subset of
  #   Cie10 fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Cie10>, Array] the matching Cie10 items; raises Cie10Error on failure
