...
module Cie10Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = Cie10Context.new({}, nil)
    end
    op = ctx.op || Cie10Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || Cie10Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(Cie10Error) ? err.msg : err.to_s
    ...
    sdk_err = Cie10Error.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(Cie10Error)
