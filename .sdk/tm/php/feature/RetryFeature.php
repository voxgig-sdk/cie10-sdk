...
class Cie10RetryFeature extends Cie10BaseFeature
{
    ...
    public function init(Cie10Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Cie10Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Cie10Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Cie10Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
