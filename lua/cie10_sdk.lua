...
Cie10SDK._make_feature = _make_feature


function Cie10SDK.new(options)
  local self = setmetatable({}, Cie10SDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function Cie10SDK:get_utility()
  return Utility.copy(self._utility)
end


function Cie10SDK:get_root_ctx()
  return self._rootctx
end


function Cie10SDK:prepare(fetchargs)
...
function Cie10SDK:direct(fetchargs)
...
-- by passing it.
function Cie10SDK:_raw_request(fetchargs)
  local utility = self._utility
...
function Cie10SDK:graphql(query, variables, ctrl)
...
-- Idiomatic facade: client:Cie10():list() / client:Cie10():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function Cie10SDK:Cie10(data)
  local EntityMod = require("entity.cie_10_entity")
  if data == nil then
    if self._cie_10 == nil then
      self._cie_10 = EntityMod.new(self, nil)
    end
    return self._cie_10
  end
...
function Cie10SDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = Cie10SDK.new(sdkopts)
  sdk.mode = "test"
